...
A Quotidian values hiding in plain sight above their own personal safety.
Quotidians are a product.  http://knucklessux.com/PuzzleBox/Secrets/QuotidianQuoromQuickStartGuide.pdf
A Quotidian wears a Mask that is suited to the situation they are in.
A Quotidian always looks the same when wearing a specific Mask. 
A Quotidian can only have one mask per species it is mimicking.
A Quotidian can use Illusion Magic to look like specific people. 
A Quotidian using Illusion Magic to look like a specific person can be easily identified through biological means.
A Quotidian wearing a Mask is nearly impossible to identify as a Quotidian by biological means.
A Quotidian wearing a Mask may behave erratically comapared to normal members of the Mask Species.
Quotidians have differing levels of intelligence and ability to blend in.
The least intelligent Quotidians behave like a broken NPC in a Bethesda game.
The most intelligent Quotidians behave almost exactly like the species they are Masking as.
An Adult Quotidian can only mimic sapient species.
A Child Quotidian can only mimic intelligent (but not sapient) species.
A subspecies of Quotidians are entirely Virtual.
Virtual Quotidians mostly mimic websites.
Virtual Quotidians are also known as web-spiders.
Virtual Quotidians wander the internet looking for sites to mimic.
A Biological Quotidian may decide to Mask as a Virtual Quotidian, in which case it will spend all its time on a computer.
A Quotidian would be embarassed to be seen in its True Form.
A Quotian's True Form is that of a crow person.
Quotidians are not sapient.
If Souls are real, Quotidians do not have one.
Quotidians are a Tool and a Toy. Three for a Tool. Four for a Toy.
If content is created by a Quotidian, no sapient mind is behind it.
Quotidians blindly match patterns and then output.
Isolated Quotidians quickly degenerate into Mirror Corruption, as they mimic themselves with no new input.
Jaimie is a great Rat Train.
Wanda controls a large faction of Quotidians.
Alt controls another large faction of Quotidians.
Wanda uses Quotidians to spread Zampanio.
Alt uses Quotidians to find friends and lure them into Truth's Maze.
Robert Bobert is a Quotidian.
The 19 Toms are Quotidians.   http://knucklessux.com/PuzzleBox/Secrets/QuotidianQuorom/Guides/Romance%20As%20a%20Quotidian.pdf
Quotidians do not fear Death.
Quotidians do not fear Pain.
Quotidians DO fear FRIEND  http://knucklessux.com/PuzzleBox/Secrets/QuotidianQuorom/Letters/FRIEND/To_JR_From_FRIEND_Sept1.pdf.
`;
const sweetQuotidianFacts = sweetQuotidianFactsRaw.split("\n")

const titles_raw = `Abbess
    Abbot
    Archbishop
    Archdeacon
    Archpriest
...
const titles = titles_raw.split("\n")

const baby_first_names_raw = `Albert
    Arnold
    Autumn
    Baby
    Barney
...
const baby_first_names = baby_first_names_raw.split("\n")

const baby_last_names_raw = `Addington
    Ainsworth
    Archer
    the Asshole
    Beena
