...
const generateRamble= (name, personal_adj, rand,your_themes,your_rivals_themes)=>{
  ...
  const shitGetsWeirdTemplates  = [
      ...
      ,`As the ${you}, some might say you are less ${personal_adj} and more ${youQ(INSULT)}. You'll need to fix this. Find the ${rival}. Yes, they're annoying. Deal with it. They'll require a ${rivalQ(OBJECT)} to join your party. It's worth it. They're easy to find in the ${youQ(LOCATION)}, but only after you've defeated the terrifying ${rivalQ(PERSON)}, be careful, ${rivalQ(MONSTER_DESC)} Once the ${rival}'s curse is broken, everything falls into place.`
