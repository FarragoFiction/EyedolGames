...
const generateRamble= (name, personal_adj, rand,your_themes,your_rivals_themes)=>{
  ...
  const shitGetsWeirdTemplates  = [
      ...
      `
      ,`You will meet the ${rival}. They ${rivalQ(rand, GENERALBACKSTORY)}. You'll hate it. Try to ignore it for now. You'll need to work with them long enough to placate the ${youQ(rand, PERSON)}s. My suggestion is try using any ${youQ(rand, OBJECT)} you find a way, especially if its ${youQ(rand, ADJ)}.`
