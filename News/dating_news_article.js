...
class DatingArticle {
  ...
  handleFirstSection = (ele) => {
    ...
    const explanations = [`A victim is lured to what ${this.rand.pickFrom(["at first glance", "initially", ""])} appears to be a dating site. ${this.rand.pickFrom(["It is unclear how the victims vanish", "Authorities have not released further details", "It is unknown how the criminals gain access to the victims"])}, but logs show nonsensical, spiralling conversations with clear bot accounts.`, `Logs from the victims accounts show spiralling non-sensical conversations with seeming chat bots preceding their disappearnce. `, `Victims often leave cryptic messages behind speaking of '${this.rand.pickFrom(["the truth is layered", "the end is never the end", "needing to dig further"])}'.`, `Victims become increasingly certain that there is deeper meaning in seemingly ordinary websites.`];

    intro.innerHTML = `
      <p>${this.rand.pickFrom(["A", "In breaking news, a", "Tragedy once again befalls our area as a"])} ${this.rand.pickFrom(["local single", "local resident", "local shut in", "person of interest"])} has ${this.rand.pickFrom(["gone missing", "last been seen", "not been seen since"])} Friday, after ${this.rand.pickFrom(["friends", "neighbors", "family members", "coworkers", "classmates"])} reported them ${this.rand.pickFrom(["clicking a link to", "spending more and more time on", "becoming increasingly obsesed with"])} a dating site called 'jackElope'. ${this.rand.pickFrom(["They have been missing over 48 hours.", "They are presumed dead.", ""])}</p>
