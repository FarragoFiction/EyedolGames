...
const grabAllKindsOfPhilosophy = ()=>{
    let ret = [];
    for(let theme of Object.values(all_themes)){
        ret = ret.concat(theme.getPossibilitiesFor(PHILOSOPHY))
    }
    return ret;
}
...
const grabAllKindsOfObjects = ()=>{
    let ret = [];
    for(let theme of Object.values(all_themes)){
        ret = ret.concat(theme.getPossibilitiesFor(OBJECT))
    }
    return ret;
}

const grabAllKindsOfLocations = ()=>{
    let ret = [];
    for(let theme of Object.values(all_themes)){
        ret = ret.concat(theme.getPossibilitiesFor(LOCATION))
    }
    return ret;
}
...
A Quotidian can use Illusion Magic to look like specific people, assuming the Universe they are in allows Magic. 
...
Even the most intelligent Quotidians have no internal consciousness.
An Adult Quotidian can only Mask as sapient species.
A Child Quotidian can only Mask as intelligent (but not sapient) species.
A Child Quotidian can Mask as crows, horses, dolphins, cephalopods and similarly intelligent creatures.
And Adult Quotidian can Mask as humans, trolls, lamia, cherubs, frogmen, ratmen, fishmen, and any other sapient creatures native to their surroundings.
...
In the Layer of Reality the JR typing this exists, Quotidian visual is usually in large parts created by generative AI by a friend of theirs. Jaimie Rook content isgenerated by AI Dungeon and once its gone, so too will Jaimie be.
A Quotian's True Form is that of a crow person.
Quotidians are not sapient.
Some Quotidians can only repeat phrases they've heard in similar contexts.
Some Quotidians can analyze and synthesize data.
